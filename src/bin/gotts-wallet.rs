...
//! Main for building the binary of a Gotts Reference Wallet
...
use gotts_wallet_util::init_logger;
...
fn real_main() -> i32 {
	...
	if !cmd::seed_exists(w.members.as_ref().unwrap().wallet.clone()) {
		if "init" == args.subcommand().0 || "recover" == args.subcommand().0 {
			...
			println!("Wallet seed file doesn't exist. Run `gotts-wallet init` first");
			exit(1);
		}
	}

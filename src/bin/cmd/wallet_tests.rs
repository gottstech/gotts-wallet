...
mod wallet_tests {
	...
	use gotts_wallet_config::{GlobalWalletConfig, WalletConfig};
	use gotts_wallet_impls::{LMDBBackend, WalletSeed};
	use gotts_wallet_libwallet::{WalletBackend, WalletInst};
	use gotts_wallet_util::gotts_core::global::{self, ChainTypes};
	...
	pub fn config_command_wallet(
		dir_name: &str,
		wallet_name: &str,
	) -> Result<(), gotts_wallet_controller::Error> {
		let mut current_dir;
		...
		config_file_name.push("gotts-wallet.toml");
		if config_file_name.exists() {
			return Err(gotts_wallet_controller::ErrorKind::ArgumentError(
				"gotts-wallet.toml already exists in the target directory. Please remove it first"
					.to_owned(),
			))?;
		}
	...
	fn execute_command(
		...
		arg_vec: Vec<&str>,
	) -> Result<String, gotts_wallet_controller::Error> {
	...
	/// command line tests
	fn command_line_test_impl(test_dir: &str) -> Result<(), gotts_wallet_controller::Error> {
		setup(test_dir);
		...
		let yml = load_yaml!("../gotts-wallet.yml");
		let app = App::from_yaml(yml);

		// wallet init
		let arg_vec = vec!["gotts-wallet", "-p", "password", "init", "-h"];
		...
		let arg_vec = vec!["gotts-wallet", "-p", "password", "account", "-c", "mining"];
		...
		let arg_vec = vec!["gotts-wallet", "-p", "password", "account"];
		execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
		...
		let wallet1 = instantiate_wallet(config1.clone(), client1.clone(), "password", "default")?;
		gotts_wallet_controller::controller::owner_single_use(wallet1.clone(), |api| {
		...
		let arg_vec = vec!["gotts-wallet", "-p", "password", "-a", "mining", "info"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
		...
		let arg_vec = vec!["gotts-wallet", "-p", "password", "-a", "account_1", "info"];
		execute_command(&app, test_dir, "wallet2", &client1, arg_vec)?;

		// check results in wallet 2
		let wallet2 = instantiate_wallet(config2.clone(), client2.clone(), "password", "default")?;
		gotts_wallet_controller::controller::owner_single_use(wallet2.clone(), |api| {
		...
		let arg_vec = vec!["gotts-wallet", "-p", "password", "-a", "mining", "txs"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
		...
		// txs and outputs (mostly spit out for a visual in test logs)
		let arg_vec = vec!["gotts-wallet", "-p", "password", "-a", "mining", "outputs"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

		let arg_vec = vec!["gotts-wallet", "-p", "password", "txs"];
		execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;

		let arg_vec = vec!["gotts-wallet", "-p", "password", "outputs"];
		execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;

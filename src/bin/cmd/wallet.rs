...
use gotts_wallet_impls::{HTTPNodeClient, SEED_FILE};
use gotts_wallet_libwallet::NodeClient;
use semver::Version;
use std::path::PathBuf;
use std::thread;
use std::time::Duration;

const MIN_COMPAT_NODE_VERSION: &str = "0.0.1";
...
pub fn wallet_command(wallet_args: &ArgMatches<'_>, config: GlobalWalletConfig) -> i32 {
	...
	if let Some(v) = node_client.clone().get_version_info() {
		// Isn't going to happen just yet (as of 0.0.1) but keep this here for
		// the future. the nodeclient's get_version_info will return 0.0.0 if
		// it gets a 404 for the version function
		if Version::parse(&v.node_version) < Version::parse(MIN_COMPAT_NODE_VERSION) {
			println!("Specified Gotts Node (version {}) is outdated and incompatible with this wallet version", v.node_version);

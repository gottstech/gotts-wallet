...
lazy_static! {
	pub static ref GOTTS_OWNER_BASIC_REALM: HeaderValue =
		HeaderValue::from_str("Basic realm=GottsOwnerAPI").unwrap();
}
		...
		_ => {
			...
			if let Some(s) = slate {
				if s.version_info.version < CURRENT_SLATE_VERSION
					...
					|| (bhv > 1 && s.version_info.block_header_version < GOTTS_BLOCK_HEADER_VERSION)
				{
	...
	K: Keychain + 'static,
{
	...
	if api_secret.is_some() {
		let api_basic_auth =
			"Basic ".to_string() + &to_base64(&("gotts:".to_string() + &api_secret.unwrap()));
		let basic_auth_middleware = Arc::new(BasicAuthMiddleware::new(
			api_basic_auth,
			&GOTTS_OWNER_BASIC_REALM,
		));

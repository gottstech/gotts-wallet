...
impl WalletSeed {
	...
	pub fn recover_from_phrase(
		...
		if !Path::new(data_file_dir).exists() {
			return Err(ErrorKind::WalletDoesntExist(
				...
				"To create a new wallet from a recovery phrase, use 'gotts-wallet init -r'"
					.to_owned(),
			))?;
		}
	...
	pub fn from_file(data_file_dir: &str, password: &str) -> Result<WalletSeed, Error> {
		...
		if Path::new(seed_file_path).exists() {
			...
			error!(
				"wallet seed file {} could not be opened (gotts wallet init). \
				 Run \"gotts wallet init\" to initialize a new wallet.",
				seed_file_path
			);

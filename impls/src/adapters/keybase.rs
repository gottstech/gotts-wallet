...
const SLATE_NEW: &str = "gotts_slate_new";
const SLATE_SIGNED: &str = "gotts_slate_signed";
...
fn poll(nseconds: u64, channel: &str) -> Option<Slate> {
	...
	error!(
		"No response from @{} in {} seconds. Gotts send failed!",
		channel, nseconds
	);
	None
}
	...
	if keybase_notify_ttl > 0 {
		...
		if let Ok(username) = my_username {
			...
			if vec.len() > 1 {
				...
				info!(
					"tx from @{} is done, please check on gotts wallet. tx uuid: {}",
					sender, tx_uuid,
				);
			}
		} else {

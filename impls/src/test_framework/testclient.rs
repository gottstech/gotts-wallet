...
where
	...
	K: Keychain,
{
	/// Create a new client that will communicate with the given gotts node
	pub fn new(chain_dir: &str) -> Self {
		set_mining_mode(ChainTypes::AutomatedTesting);
		let genesis_block = pow::mine_genesis_block().unwrap();
		let verifier_cache = Arc::new(RwLock::new(LruVerifierCache::new()));
		let dir_name = format!("{}/.gotts", chain_dir);
		let c = Chain::init(
			dir_name.to_string(),
			Arc::new(NoopAdapter {}),
			genesis_block,
	...
	fn send_tx_slate(
		...
		let slate: Slate = {
			...
			// receive tx
			foreign::receive_tx(&mut *w, &Slate::from(slate), None, None, false)?
		};

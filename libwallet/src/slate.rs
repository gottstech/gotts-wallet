...
impl fmt::Display for ParticipantMessageData {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		...
		let message_sig = match self.message_sig.clone() {
			None => "None".to_owned(),
			Some(m) => gotts_util::to_hex(m.to_raw_data().to_vec()),
		};
...
pub struct VersionCompatInfo {
	...
	/// The gotts block header version this slate is intended for
...
impl Slate {
	...
	pub fn blank(num_participants: usize) -> Slate {
		Slate {
			...
			version_info: VersionCompatInfo {
				...
				block_header_version: GOTTS_BLOCK_HEADER_VERSION,
			},
		}
	}
		...
		K: Keychain,
	{
		...
		let _ = final_tx.validate(Weighting::AsTransaction, verifier_cache, None, self.height)?;
...
impl SlateVersionProbe {
	/// Get version number (an u16 integer)

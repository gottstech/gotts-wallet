...
const FILE_LOGGING_PATTERN: &str = "{d(%Y%m%d %H:%M:%S%.3f)} {h({l})} {M} - {m}{n}";
const STDOUT_LOGGING_PATTERN: &str = "{d(%m%d %H:%M:%S)} {h({l})} - {m}{n}";

/// This filter is rejecting messages that doesn't start with "gotts"
/// in order to save log space for only Gotts-related records
#[derive(Debug)]
struct GottsFilter;

impl Filter for GottsFilter {
	fn filter(&self, record: &Record<'_>) -> Response {
		if let Some(module_path) = record.module_path() {
			if module_path.starts_with("gotts") {
	...
	if let Some(c) = config {
		...
		*config_ref = c.clone();
		...
		let stdout = ConsoleAppender::builder()
			.encoder(Box::new(PatternEncoder::new(&STDOUT_LOGGING_PATTERN)))
			.build();
				...
				if let Some(size) = c.log_max_size {
					...
					Box::new(
						RollingFileAppender::builder()
							...
							.encoder(Box::new(PatternEncoder::new(&FILE_LOGGING_PATTERN)))
